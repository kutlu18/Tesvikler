import { KalkinmaSupportResult } from "../types";

interface KalkinmaAjansiResultCardProps {
  result: KalkinmaSupportResult;
}

const statusStyles: Record<KalkinmaSupportResult["status"], string> = {
...
export default function KalkinmaAjansiResultCard({ result }: KalkinmaAjansiResultCardProps) {
  return (
    <article className={`rounded-2xl border p-5 shadow-sm transition hover:shadow-card ${statusStyles[result.status]}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="rounded-xl bg-white px-3 py-2 text-xs text-slate-700 shadow-sm">
          ...
          <p>
            <span className="font-semibold">Destek sınıfı:</span> {result.supportClass}
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <div className="space-y-3">
          <p className="text-sm">
            <span className="font-semibold text-slate-800">Kimler başvurabilir?</span> {result.applicantProfile}
          </p>
          <p className="text-sm">
            <span className="font-semibold text-slate-800">Hangi proje tipi için uygundur?</span> {result.suitableProjectType}
          </p>
          <p className="text-sm">
            <span className="font-semibold text-slate-800">Desteklenen giderler:</span> {result.supportedExpenses}
          </p>
          <p className="text-sm">
          ...
          <ListBlock title="Nasıl alınır?" items={result.howToGet} />
        </div>
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm">
            <span className="font-semibold text-slate-800">Başvuru kanalı:</span> {result.applicationChannel}
          </p>
          <p className="text-sm">
