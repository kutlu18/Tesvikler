import { EximbankSupportResult } from "../types";

interface EximbankResultCardProps {
  result: EximbankSupportResult;
}

const statusStyles: Record<EximbankSupportResult["status"], string> = {
...
export default function EximbankResultCard({ result }: EximbankResultCardProps) {
  return (
    <article className={`rounded-2xl border p-5 shadow-sm transition hover:shadow-card ${statusStyles[result.status]}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="rounded-xl bg-white px-3 py-2 text-xs text-slate-700 shadow-sm">
          ...
          <p>
            <span className="font-semibold">Ürün sınıfı:</span> {result.productClass}
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <div className="space-y-3">
          ...
          <p className="text-sm">
            <span className="font-semibold text-slate-800">Hangi ihtiyaç için uygundur?</span> {result.suitableNeed}
          </p>
          <p className="text-sm">
            <span className="font-semibold text-slate-800">Finansman konusu:</span> {result.financeSubject}
          </p>
          <ListBlock title="Neden uygun?" items={result.whyEligible} />
          <ListBlock title="Finansman nasıl alınır?" items={result.howToGet} />
        </div>
